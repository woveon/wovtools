...
tr_verbose 
{
  ...
  tr_run "shut down any local postgres server" \
    "wov-db --context here:${USECLUSTER}-${PROJ}-${TESTME} Adb --stop"
  tr_run "bring up a local postgres server" \
    "wov-db --context here:${USECLUSTER}-${PROJ}-${TESTME} --docker-postgres-start Adb"
  tr_test "create the database" \
    "wov-db --context here:${USECLUSTER}-${PROJ}-${TESTME} --wdb-createinstance Adb" 0 -1
...
#  tr_run "asdf" "wov-push-db --snapshot X"
  ...
  tr_test "check in db deltas" \
    "wov-push-db --snapshot X Adb" 0 -1
  ...
  tr_test "change snapshot" \
    "wov-push-db --snapshot Y Adb" 0 -1
