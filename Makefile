...
BINS-DB=wov-db-common wov-db-common-helm wov-db-common-docker wov-db-common--postgres
BINS-CLUSTER=wov-cluster wov-cluster-make_key wov-cluster-make_cluster wov-cluster-createdb \
	           wov-cluster-configdbnet wov-kops wov-cluster-dns
BINS-ENV=wov-env-ops wov-env-build wov-env wov-env-val wov_stage-select wov-git-check wov-env-provider-common
...
BINS=${BINS-EXTERNAL} ${BINS-INIT} ${BINS-DB} ${BINS-CLUSTER} ${BINS-ENV} ${BINS-CLI} ${BINS-VH} ${BINS-PUSH} ${BINS-DEPLOY}
